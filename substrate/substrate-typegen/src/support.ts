...
export class UnknownVersionError extends Error {
    constructor(name: string) {
        super(`Item "${name}" has unknown version`)
    }
}
...
export class VersionedItem<V extends Record<string, VersionedType>> {
    ...
    constructor(
        ...
        protected UnknownVersionError: new (...args: any[]) => Error = UnknownVersionError,
        protected NonExistentItemError: new (...args: any[]) => Error = NonExistentItemError,
    ) {
        this.isExists = isExists.bind(this)
    }

    at<T>(block: RuntimeCtx, match: (this: this, ...args: GetVersionedItemEntries<V>) => T): T {
        this.at = this.createMatchVersion()
        return this.at(block, match)
    }
    ...
    private createMatchVersion(): any {
        ...
        for (let key in this.versions) {
            ...
            body += `if (${version}.is(block)) return match.call(this, ${version}.at(block), '${key}')\n`
        }
        ...
        return new Function('block', 'match', 'fallback', body)
    }
}
